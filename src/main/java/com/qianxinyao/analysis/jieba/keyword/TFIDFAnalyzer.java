...
public class TFIDFAnalyzer {
    ...
    public TFIDFAnalyzer(String configRoot) {
    ...
    public static void setConfigRoot(String configRootPath) {
    ...
    private static void loadIDFMap(boolean reload) throws IOException {
        if (idfMap == null || reload) {
            idfMap = new HashMap<>();
            loadIDFMap(idfMap, Files.newInputStream(Paths.get(configRoot, "idf_dict.txt")));
        }
    }

    private static void loadStopWords(boolean reload) throws IOException {
        if (stopWordsSet == null || reload) {
            stopWordsSet = new HashSet<>();
            loadStopWords(stopWordsSet, Files.newInputStream(Paths.get(configRoot, "stop_words.txt")));
        }
    }
    ...
    private Map<String, Double> getTF(String content) {
        ...
        JiebaSegmenter segmenter = new JiebaSegmenter(getConfigRoot());
        if (getConfigRoot() != null && getConfigRoot().trim().length() > 0) {
            segmenter.initUserDict(Paths.get(getConfigRoot()));
        }
    ...
    public  static void reloadConfig(){
        if(configRoot!=null && !configRoot.equals("")){
            reloadConfig(configRoot);
        }
    }
    public  static void reloadConfig(String configRootFolder){
        try {
            configRoot=configRootFolder;
            loadIDFMap(true);
            loadStopWords(true);
            JiebaSegmenter.reloadConfig(configRootFolder);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
